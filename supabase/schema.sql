...
create unique index orders_vendor_order_number_day_idx on public.orders (vendor_id, order_number, (created_at::date));
...
begin
  ...
  from orders
  where vendor_id = v_vendor_id and created_at::date = current_date;

  v_order_number := case when p_vendor_slug like '%riceball%' then 'RB' else 'CP' end || lpad(v_order_count::text, 3, '0');

  insert into orders (vendor_id, order_number, customer_name, phone_last4, pickup_time, payment_method, total_amount)
  values (v_vendor_id, v_order_number, p_customer_name, p_phone_last4, p_pickup_time, p_payment_method, v_total)
